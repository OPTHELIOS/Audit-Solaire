...
from services.evidence_service import save_cover_photo
...
def _save_dossier(audit) -> None:
    ...
    uploaded_cover = st.session_state.get("dossier_photo_couverture_uploader")
    if uploaded_cover is not None:
        audit.projet.photo_couverture_path = save_cover_photo(audit, uploaded_cover)
...
def render() -> None:
    ...
    st.subheader("Photo de couverture (page de garde du rapport)")
    st.caption(
        "Photo « vitrine » du site affichée en page de garde du rapport DOCX — distincte "
        "des preuves ajoutées dans « Preuves et annexes », qui documentent des constats précis."
    )

    current_photo = audit.projet.photo_couverture_path
    if current_photo and Path(current_photo).exists():
        st.image(current_photo, caption="Photo de couverture actuelle", width=300)

    st.file_uploader(
        "Remplacer la photo de couverture",
        type=["jpg", "jpeg", "png"],
        key="dossier_photo_couverture_uploader",
        help="Sera enregistrée au clic sur « Enregistrer le dossier » ci-dessous.",
    )
