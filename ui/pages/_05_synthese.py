...
def _render_expert_conclusion(payload: dict[str, Any]) -> None:
    """Conclusion experte libre.

    CORRECTIF : la conclusion n'était enregistrée que dans
    `st.session_state[SESSION_CONCLUSION_KEY]`, jamais dans l'objet `Audit`
    (`audit.synthese.conclusion_generale`). Résultat : rouvrir un audit
    sauvegardé faisait disparaître la conclusion saisie, puisqu'elle ne
    survivait qu'à la session Streamlit en cours. Elle est maintenant
    écrite dans le modèle et persistée via `save_audit`, donc elle survit à
    la fermeture de l'onglet et à la sauvegarde cloud.
    """
    ...
    audit = get_audit()
    default_text = (
        st.session_state.get(SESSION_CONCLUSION_KEY, "")
        or audit.synthese.conclusion_generale
