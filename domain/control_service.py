...
def append_uploaded_evidences(
    ...
    """Ecrit localement les fichiers uploades pour ce point de controle.

    Retourne `(all_paths, new_paths)` : `all_paths` est la liste complete
    (anciens + nouveaux, pour `update_response`/`constat.photos`) et
    `new_paths` ne contient que les fichiers ecrits lors de CET appel — utile
    a l'appelant pour ne renvoyer vers le cloud que les fichiers reellement
    nouveaux (voir `services.evidence_service.backup_control_evidence_file`),
    sans re-uploader inutilement des preuves deja sauvegardees."""
