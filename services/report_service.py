...
def _make_impact_sentence(finding: dict[str, Any]) -> str:
    ...
    return (
        "L’impact potentiel doit être apprécié au regard du fonctionnement, "
        "de la sécurité et de la performance de l’installation."
    )
...
def _make_action_sentence(finding: dict[str, Any]) -> str:
    ...
    return (
        "Action recommandée : définir puis mettre en œuvre une action corrective "
        "adaptée au défaut observé."
    )
...
def _build_executive_summary(summary: dict[str, Any], actions: list[dict[str, Any]]) -> list[str]:
    ...
    return [
        ...
        (
            f"La campagne de contrôle met en évidence {nc} non-conformité(s), "
            f"{np} point(s) non présent(s), {nv} point(s) non vérifiable(s) "
            f"et {nr} point(s) restant à renseigner."
        ),
        (
            f"Le plan d’actions issu de l’audit comprend {p1} action(s) de priorité P1, "
            f"{p2} action(s) de priorité P2 et {p3} action(s) de priorité P3."
        ),
    ]
...
def _build_global_assessment(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    commentaire = (
        "L’installation présente un niveau globalement acceptable, "
        "sous réserve du traitement des écarts relevés."
    )

    if nb_crit > 0:
        statut = "défavorable"
        commentaire = (
            "L’installation présente au moins un écart critique nécessitant "
            "un traitement prioritaire avant considération d’un fonctionnement satisfaisant."
        )
    elif nb_maj >= 3:
        statut = "réserves majeures"
        commentaire = (
            "L’installation présente plusieurs écarts majeurs susceptibles "
            "d’altérer durablement la performance, la disponibilité ou la maintenabilité."
        )
    elif taux_completion < 90:
        statut = "à consolider"
        commentaire = (
            "L’analyse reste partiellement incomplète et nécessite la consolidation "
            "des points encore non renseignés ou non vérifiables."
        )
...
def _build_key_messages(findings: list[ReportFinding], actions: list[dict[str, Any]]) -> list[str]:
    ...
    if critical_findings:
        messages.append(
            "Des écarts critiques ont été identifiés et doivent être traités en priorité "
            "avant toute appréciation favorable du fonctionnement global."
        )
    ...
    if any(a["priorite"] == "P1" for a in actions):
        messages.append(
            "Le plan d’actions comprend des mesures de priorité P1 nécessitant "
            "un traitement prioritaire et tracé."
        )

    if not messages:
        messages.append(
            "Aucun message d’alerte majeur ne ressort à ce stade, sous réserve de compléter "
            "et confirmer l’ensemble des points de contrôle."
        )
